...
export function DepositView({ account, depositDetails }: DepositViewProps) {
  ...
  const tenureCompletion = useMemo(() => {
    ...
    return (depositDetails.completed_months / depositDetails.tenure_months) * 100;
  }, [depositDetails]);

  // Calculate returns percentage
  const returnsPercentage = useMemo(() => {
    if (!depositDetails?.principal_amount) return 0;
    return ((depositDetails.total_interest_earned / depositDetails.principal_amount) * 100);
  }, [depositDetails]);
  ...
  const getMaturityStatus = () => {
    ...
    if (daysUntilMaturity <= 30) return { text: 'Maturing Soon', color: 'warning' };
      ...
      <div className="deposit-view">
        <Card>
          <div className="deposit-view__empty">
            ...
            <p>Add deposit details to see interest rates, maturity information, and returns.</p>
          </div>
        </Card>
      </div>
    );
    ...
    <div className="deposit-view">
      ...
      <Card className="deposit-view__maturity-card">
        <div className="deposit-view__maturity-header">
          <div>
            ...
            <span className={`deposit-view__status deposit-view__status--${maturityStatus.color}`}>
              {maturityStatus.text}
            </span>
          </div>
      ...
      <div className="deposit-view__stats">
        <StatCard
          ...
          value={formatCurrency(depositDetails.principal_amount, account.currency)}
        ...
        <StatCard
          ...
          value={formatCurrency(depositDetails.total_interest_earned, account.currency)}
          icon={<TrendingUp size={20} />}
          variant="success"
        />
      ...
      <Card>
        ...
        <div className="deposit-view__details">
          ...
          <div className="deposit-view__detail-row">
            <span>Tenure</span>
            <span className="deposit-view__detail-value">
              {depositDetails.tenure_months} months ({Math.floor(depositDetails.tenure_months / 12)} years)
            </span>
          </div>
          ...
          <div className="deposit-view__detail-row">
            ...
            <span className="deposit-view__detail-value">
              {depositDetails.interest_payout_frequency?.replace('_', ' ').charAt(0).toUpperCase() + 
               depositDetails.interest_payout_frequency?.slice(1) || 'Not specified'}
            </span>
          </div>
          ...
          <div className="deposit-view__tax">
            {depositDetails.is_tax_saving && (
              <div className="deposit-view__tax-item">
                ...
                <div>
                  <span className="deposit-view__tax-label">Tax Saving Scheme</span>
            ...
            {depositDetails.tds_deducted > 0 && (
              <div className="deposit-view__tax-item">
                ...
                <div>
                  ...
                  <span className="deposit-view__tax-value">
                    {formatCurrency(depositDetails.tds_deducted, account.currency)}
                  </span>
                </div>
              </div>
            )}
      ...
      {/* Institution Details */}
      {(depositDetails.bank_name || depositDetails.branch || depositDetails.account_number) && (
        <Card>
          ...
          <div className="deposit-view__nominee">
            ...
            <div className="deposit-view__nominee-details">
              {depositDetails.nominee_name && (
                <div>
                  <span className="deposit-view__nominee-label">Nominee Name</span>
              ...
              {depositDetails.nominee_relationship && (
                <div>
                  <span className="deposit-view__nominee-label">Relationship</span>
                  <span className="deposit-view__nominee-value">
                    {depositDetails.nominee_relationship}
                  </span>
                </div>
      ...
      <Card>
        ...
        <div className="deposit-view__features">
          <div className={`deposit-view__feature ${depositDetails.auto_renewal ? 'deposit-view__feature--active' : ''}`}>
          ...
          <div className={`deposit-view__feature ${depositDetails.premature_withdrawal_allowed ? 'deposit-view__feature--active' : ''}`}>
            <div className="deposit-view__feature-icon">
          ...
          <div className={`deposit-view__feature ${depositDetails.loan_against_deposit_allowed ? 'deposit-view__feature--active' : ''}`}>
            <div className="deposit-view__feature-icon">
