...
import { Calendar, CreditCard, DollarSign, Gift, TrendingDown, TrendingUp } from 'lucide-react';
...
export function CreditCardView({ account, creditCardDetails }: CreditCardViewProps) {
      ...
      <div className="credit-card-view">
        <Card>
          <div className="credit-card-view__empty">
            ...
            <p>Add credit card details to see limit, billing cycle, and rewards information.</p>
          </div>
        </Card>
      </div>
    );
    ...
    <div className="credit-card-view">
      ...
      <div className="credit-card-view__header">
        ...
        {creditCardDetails.issuer_bank && (
          <span className="credit-card-view__bank">{creditCardDetails.issuer_bank}</span>
        )}
      </div>
      ...
      <Card className="credit-card-view__limit-card">
        ...
        <div className="credit-card-view__utilization">
          <div className="credit-card-view__utilization-header">
            ...
            <span className={`credit-card-view__utilization-percent credit-card-view__utilization-percent--${getUtilizationColor(creditUtilization)}`}>
          ...
          <div className="credit-card-view__utilization-details">
            <span>Used: {formatCurrency(creditCardDetails.current_balance, account.currency)}</span>
            <span>Available: {formatCurrency(creditCardDetails.available_credit, account.currency)}</span>
          </div>
        </div>
      </Card>
      ...
      <div className="credit-card-view__stats">
        <StatCard
          ...
          value={formatCurrency(creditCardDetails.current_balance, account.currency)}
          icon={<DollarSign size={20} />}
        />
        
        <StatCard
          label="Minimum Due"
          value={formatCurrency(creditCardDetails.minimum_due, account.currency)}
      ...
      <Card>
        ...
        <div className="credit-card-view__billing">
          <div className="credit-card-view__billing-item">
            <Calendar size={20} />
            <div>
              <span className="credit-card-view__billing-label">Billing Cycle Day</span>
          ...
          {creditCardDetails.statement_date && (
            <div className="credit-card-view__billing-item">
              <Calendar size={20} />
              <div>
                <span className="credit-card-view__billing-label">Last Statement Date</span>
                <span className="credit-card-view__billing-value">
          ...
          {creditCardDetails.payment_due_date && (
            <div className="credit-card-view__billing-item">
              <Calendar size={20} />
              <div>
                <span className="credit-card-view__billing-label">Payment Due Date</span>
                <span className="credit-card-view__billing-value">
                  {formatDate(creditCardDetails.payment_due_date)}
                  {daysUntilDue !== null && (
                    <span className={`credit-card-view__days-until ${daysUntilDue <= 3 ? 'credit-card-view__days-until--urgent' : ''}`}>
      ...
      <Card>
        ...
        <div className="credit-card-view__details">
          ...
          <div className="credit-card-view__detail-row">
            <span>Autopay</span>
            <span className={`credit-card-view__detail-badge ${creditCardDetails.autopay_enabled ? 'credit-card-view__detail-badge--active' : ''}`}>
      ...
      {/* Rewards & Cashback */}
      {(creditCardDetails.cashback_earned > 0 || creditCardDetails.rewards_points > 0) && (
        <Card>
          ...
          <div className="credit-card-view__rewards">
            {creditCardDetails.cashback_earned > 0 && (
              <div className="credit-card-view__reward-item">
                ...
                <div>
                  <span className="credit-card-view__reward-label">Total Cashback Earned</span>
                  <span className="credit-card-view__reward-value">
                    {formatCurrency(creditCardDetails.cashback_earned, account.currency)}
                  </span>
                </div>
              </div>
            )}
            ...
            {creditCardDetails.rewards_points > 0 && (
              <div className="credit-card-view__reward-item">
                <div className="credit-card-view__reward-icon">🎁</div>
                <div>
                  <span className="credit-card-view__reward-label">Rewards Points Balance</span>
                  <span className="credit-card-view__reward-value">
                  ...
                  {creditCardDetails.rewards_value > 0 && (
                    <span className="credit-card-view__reward-sublabel">
                      Estimated value: {formatCurrency(creditCardDetails.rewards_value, account.currency)}
                    </span>
                  )}
                </div>
              </div>
