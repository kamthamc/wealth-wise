...
import type { Account, AccountType, BrokerageDetails, CreditCardDetails, DepositDetails } from '@/core/db/types';
...
export function AccountViewFactory({
  ...
  if (CREDIT_CARD_TYPES.includes(account.type)) {
    return <CreditCardView account={account} creditCardDetails={creditCardDetails} />;
  }
  ...
  if (BROKERAGE_TYPES.includes(account.type)) {
    return <BrokerageView account={account} brokerageDetails={brokerageDetails} />;
  }
 ...
 */
export function getViewType(accountType: AccountType): 'credit_card' | 'deposit' | 'brokerage' | 'default' {
