...
export const useAccountStore = create<AccountState>((set, get) => ({
  ...
  createAccount: async (input) => {
    ...
    try {
      ...
      if (input.type === 'credit_card' && inputWithCreditCard.creditCardDetails) {
        const ccDetails = inputWithCreditCard.creditCardDetails;
        const { creditCardDetailsRepository } = await import('@/core/db/repositories');
        ...
        await creditCardDetailsRepository.create({
          ...
          card_network: (ccDetails.card_network || undefined) as 'visa' | 'mastercard' | 'amex' | 'rupay' | 'diners' | undefined,
          card_type: (ccDetails.card_type || undefined) as 'credit' | 'charge' | undefined,
      ...
      if (input.type === 'brokerage' && inputWithBrokerage.brokerageDetails) {
        ...
        const { brokerageDetailsRepository } = await import('@/core/db/repositories');
        ...
        await brokerageDetailsRepository.create({
          ...
          bond_holdings: 0,
          etf_holdings: 0,
          account_type: (brokDetails.account_type || undefined) as 'trading' | 'demat' | 'combined' | undefined,
          status: 'active' as const,
