 ...
 */
export function multiplyCurrency(amount: number | string, factor: number): number {
...
export function calculateNetChange(
  ...
  const income = toBigIntCents(calculateIncome(transactions, startDate, endDate));
  const expenses = toBigIntCents(calculateExpenses(transactions, startDate, endDate));
...
export function calculateMonthlyStats(
  ...
  const futureTransactions = transactions.filter(t => new Date(t.date) >= periodStart);
