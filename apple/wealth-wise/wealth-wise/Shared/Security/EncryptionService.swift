...
@MainActor
public final class EncryptionService: EncryptionServiceProtocol, ObservableObject, @unchecked Sendable {
    ...
    public func encrypt(_ data: Data, using key: SecureKey) async throws -> EncryptedData {
        ...
        do {
            ...
            guard let ciphertext = sealedBox.ciphertext,
                ...
                throw EncryptionError.encryptionFailed("Failed to extract ciphertext or tag")
            }
    ...
    public func generateRandomKey(securityLevel: SecurityLevel = .maximum) -> SecureKey {
        let keySize: SymmetricKeySize = securityLevel == .quantum ? .bits512 : .bits256
    ...
    public func encrypt(_ string: String, using key: SecureKey) throws -> EncryptedData {
    ...
    public func decryptToString(_ encryptedData: EncryptedData, using key: SecureKey) throws -> String {
    ...
    public func encryptLargeData(_ data: Data, using key: SecureKey, chunkSize: Int = 1024 * 1024) throws -> EncryptedData {
        ...
        while offset < data.count {
            ...
            let encryptedChunk = try encrypt(chunk, using: key)
        ...
        return try encrypt(combinedData, using: key)
    }
    
    /// Decrypt large data with chunking
    public func decryptLargeData(_ encryptedData: EncryptedData, using key: SecureKey) throws -> Data {
        let combinedData = try decrypt(encryptedData, using: key)
        ...
        for chunk in encryptedChunks {
            let decryptedChunk = try decrypt(chunk, using: key)
...
@available(iOS 18.6, macOS 15.6, watchOS 11.6, *)
public final class AdvancedEncryptionService: EncryptionService {
    ...
    public func generateRSAKeyPair(keySize: Int = 2048) throws -> (publicKey: SecKey, privateKey: SecKey) {
        let parameters: [String: Any] = [
            ...
            kSecPrivateKeyAttrs as String: [
                ...
                kSecAttrApplicationTag as String: "com.wealthwise.rsa.private".data(using: .utf8)!
            ],
            kSecPublicKeyAttrs as String: [
                kSecAttrIsPermanent as String: false,
                kSecAttrApplicationTag as String: "com.wealthwise.rsa.public".data(using: .utf8)!
            ]
        ]
        ...
        guard result == errSecSuccess,
            ...
            throw EncryptionError.encryptionFailed("RSA key pair generation failed")
        }
    ...
    public func hybridEncrypt(_ data: Data, using publicKey: SecKey) throws -> HybridEncryptedData {
        // Generate random AES key
        let aesKey = generateRandomKey()
        
        // Encrypt data with AES
        let encryptedData = try encrypt(data, using: aesKey)
    ...
    public func hybridDecrypt(_ hybridData: HybridEncryptedData, using privateKey: SecKey) throws -> Data {
        ...
        return try decrypt(hybridData.encryptedData, using: aesKey)
    }
...
private func CCKeyDerivationPBKDF(
    ...
    return CCKeyDerivationPBKDF(algorithm, password, passwordLen, salt, saltLen, prf, rounds, derivedKey, derivedKeyLen)
}