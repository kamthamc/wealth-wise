...
public final class AuthenticationStateManager: AuthenticationStateProtocol, @unchecked Sendable {
    ...
    deinit {
        // Cancel monitoring task synchronously
        sessionMonitoringTask?.cancel()
        // Note: Cannot call MainActor methods from deinit
        // invalidateSession() should be called manually before deallocation
    }
    ...
    private func storeSessionData(_ token: SessionToken, user: AuthenticatedUser) async throws {
        ...
        let tokenKey = try keyManager.retrieveKey(identifier: sessionTokenKey)
        ...
        let userKey = try keyManager.retrieveKey(identifier: userDataKey)
    ...
    private func validateRestoredSession(_ token: SessionToken, user: AuthenticatedUser) async throws -> Bool {
        ...
        guard payload.tokenId == token.id,
              ...
              payload.deviceId == await getDeviceIdentifier() else {
            return false
        }
    ...
    private func initializeSessionManagement() async {
        // Register background task for session monitoring
        #if os(iOS)
