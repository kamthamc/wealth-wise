...
public final class BiometricAuthenticationManager: BiometricAuthenticationProtocol, ObservableObject, @unchecked Sendable {
    ...
    @Published public private(set) var currentBiometricType: BiometricType = .none
    ...
    private let maxFailedAttempts = SecurityConfiguration.maxBiometricFailures
    private let lockoutDuration = SecurityConfiguration.biometricLockoutDuration
    ...
    public func availableBiometricType() -> BiometricType {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.currentBiometricType = biometricType
        }
    ...
    private func refreshBiometricStatus() async {
        ...
        await MainActor.run {
            self.isAvailable = available
            self.isEnrolled = enrolled
            self.currentBiometricType = type
        }
    }
    ...
    _ = try JSONEncoder().encode(result)
        ...
        // Persist token data using a symmetric key
        let key = SecureKey(
