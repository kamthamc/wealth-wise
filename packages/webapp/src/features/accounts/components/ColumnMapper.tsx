...
const SYSTEM_FIELDS = [
  ...
  {
    value: 'reference',
    label: 'Transaction Reference/ID (Chq./Ref.No.)',
    required: false,
  },
...
export function ColumnMapper({
  ...
  useEffect(() => {
    ...
    const detected = csvHeaders.map((header) => {
      ...
      // Reference/Transaction ID detection (HDFC, ICICI, SBI formats)
      else if (
        lowerHeader.includes('ref') ||
        lowerHeader.includes('reference') ||
        lowerHeader.includes('chq') ||
        lowerHeader.includes('cheque') ||
        lowerHeader.includes('transaction id') ||
        lowerHeader.includes('txn id') ||
        lowerHeader.includes('utr') ||
        lowerHeader === 'ref no' ||
        lowerHeader === 'ref.no' ||
        lowerHeader === 'chq./ref.no.'
      ) {
